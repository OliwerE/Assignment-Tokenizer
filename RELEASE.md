...
Tokenizer är den klass som tar emot ett token och en grammatik för att sedan tokenisera strängen enligt grammatiken. Tokenizer kan sedan användas för att läsa aktivt token, läsa nästa token eller läsa föregående token.
...
Koden har testats med automatiska testfall i modulen tokenizerTests. Varje testfall är en funktion som skapar ett tokenizer objekt. Sedan startar funktionen tokeniseraren genom att anropa startmetoden med en testfallets sträng som argument. Sedan anropas next/prev metoderna enligt sekvensen i testfallet och resultatet jämförs med det förväntade genom att anropa metoden getActiveToken.
